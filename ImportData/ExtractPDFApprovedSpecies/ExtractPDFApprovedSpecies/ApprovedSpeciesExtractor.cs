    ...
    public class ApprovedSpeciesExtractor : BasePDFExtractor
    {
        ...
        private void ProcessMarineSpeciesData(int class_id, int counter, string scientific, string common)
        {
            ...
            */
            ...
            command = new SqlCommand();
            command.CommandText = "SELECT ID_PK FROM [SPECIES] WHERE [SCIENTIFIC] = @SCIENTIFIC_TEXT";
            ...
            {
                insertDataCommand.CommandText = "INSERT INTO [SPECIES] " +
                    "([SCIENTIFIC], [COMMON]) VALUES (@SCIENTIFIC_TEXT, @COMMON_TEXT) ;";
                DAOHelper.InsertData(insertDataCommand);
                //record_id = DAOHelper.RetreiveID(command);
            }
            else
            {
                insertDataCommand.CommandText = "UPDATE [SPECIES] " +
                    "SET [SCIENTIFIC] = @SCIENTIFIC_TEXT, [COMMON] = @COMMON_TEXT " +
                                                " WHERE [ID_PK] = @ID_PK ;";
